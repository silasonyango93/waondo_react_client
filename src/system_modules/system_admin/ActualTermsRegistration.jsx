...
constructor(props) {
  ...
  componentWillMount(){
	  ...
	  if(UserId===null){this.props.history.push('/admin_login');}else{
	  ...
	  axios.post(ip+'/get_all_term_iterations')
		.then((response) => {
		  ...
		  my_json.forEach((item) => {
			  ...
			  jsonObject={value:item.TermIterationId,label:item.TermIterationDescription}
	...
	handleSubmit(event){ 
      ...
      axios.post(ip+"/add_term", querystring.stringify({ TermIterationId: this.state.SelectedTermIteration.value,
